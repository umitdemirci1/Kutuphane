                      ...
                      <section class="panel">
						...
						<label class="control-label col-lg-2" for="inputSuccess">Kategori Ara :</label>
							<div class="col-lg-3">
								...
								{ ?>
								<option value="aranan.php?kategore=ara&KategoriID=<?php 
								echo $kategorileri_cek['KategoriID']; ?>"><?php echo $kategorileri_cek['KategoriAdi']; ?></option>
								<?php } ?>
								</select>
							</div>
						</div>                            	
                        ...
                        <?php if(isset($_GET['kategore'])){ ?>
                        <h4>Seçilen kategori Listesi</h4>
                         <table class="table table-striped table-advance table-hover">
                           <tbody>
                              ...
                              <?php
								...
								//iki tablodan veri almak için inner join var
								$kitap_bilgi_sor=$db->prepare("SELECT * FROM kitaplartablo INNER JOIN kategoritablo ON 
								kitaplartablo.KategoriID= kategoritablo.KategoriID WHERE kitaplartablo.KategoriID=? ORDER BY KitapID DESC");
								$kitap_bilgi_sor->execute(array($KategoriID));
                              ...
                              <tr>
                                 ...
                                 <td>
                                  <a role="button" class="btn btn-success" data-toggle="modal" data-target="#myModal<?php echo $kitap_bilgi_cek['KitapID'] ?>"  >Kitabı Bilgi</a>
                                  </td>
                              </tr>
	...
	<!-- Sentence düzenle -->
                        ...
                        <?php } if(isset($_GET['aranan_kitap'])){ ?>
                        <h4>Aranan Kitaplar Listesi</h4>
                         <table class="table table-striped table-advance table-hover">
                           <tbody>
                              <tr>
                                 <th width="48%">Kitap Adı</th>
                              ...
                              <?php 
								$aranan_kelime=$_GET['aranan_kelime'];
									//iki tablodan veri almak için inner join var
								$kitap_bilgi_sor=$db->prepare("SELECT * FROM kitaplartablo INNER JOIN kategoritablo ON 
								kitaplartablo.KategoriID= kategoritablo.KategoriID WHERE kitaplartablo.KitapAdi LIKE '%$aranan_kelime%'ORDER BY KitapID DESC");
